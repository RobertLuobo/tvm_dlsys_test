...
def make_relu_gradient(shape, tgt, tgt_host, func_name, dtype="float32"):
    ...
    A = tvm.te.placeholder(shape, dtype=dtype, name="A")
    B = tvm.te.placeholder(shape, dtype=dtype, name="B")
    C = tvm.te.compute(A.shape, lambda *i: B(*i) * tvm.te.if_then_else(A(*i) > 0, 1, 0))
...
def make_matrix_mul(shapeA, transposeA, shapeB, transposeB, tgt, tgt_host,
    ...
    if not transposeA and not transposeB:
        in_a, out_a = shapeA
        in_b, out_b = shapeB
        k = tvm.te.reduce_axis((0, out_a), name='k')
        trans_b = topi.transpose(B)
        matmul = tvm.te.compute((in_a, out_b),
                             lambda i, j: tvm.te.sum(A[i, k] * trans_b[j, k], axis=k))
        ...
        trans_b = topi.transpose(B)
        matmul = tvm.te.compute((in_a, out_b),
                             lambda i, j: tvm.te.sum(trans_a[i, k] * trans_b[j, k], axis=k))
        ...
        trans_a = topi.transpose(A)
        matmul = tvm.te.compute((in_a, out_b),
                             lambda i, j: tvm.te.sum(trans_a[i, k] * B[j, k], axis=k))
...
def make_conv2d(shapeX, shapeF, tgt, tgt_host, func_name, dtype="float32"):
    ...
    """Hint: use tvm.te.reduce_axis, tvm.te.sum"""
    ...
    A = tvm.te.placeholder(shapeX, dtype=dtype, name="A")
    conv = tvm.te.placeholder(shapeF, dtype=dtype, name="conv")
    C = topi.nn.conv2d(A, conv, 1, 0, 1) # conv2d(input, filter, strides, padding, dilation)
...
def make_matrix_softmax(shape, tgt, tgt_host, func_name, dtype="float32"):
    """Hint: use tvm.te.reduce_axis, tvm.te.sum, tvm.max, tvm.exp"""
